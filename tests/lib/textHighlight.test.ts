...
describe('textHighlight utility', () => {
  describe('highlightText function', () => {
    ...
    it('should highlight multiple matching words', () => {
      const result = highlightText('Hello World Hello Universe', 'hello') as React.ReactNode[]
    ...
    it('should handle case-insensitive matching', () => {
      const result = highlightText('JavaScript Developer', 'SCRIPT') as React.ReactNode[]
      
      expect(Array.isArray(result)).toBe(true)
      const highlightedPart = result.find(part => 
    ...
    it('should handle multiple search terms', () => {
      const result = highlightText('Frontend Developer with React skills', 'Frontend React') as React.ReactNode[]
      
      expect(Array.isArray(result)).toBe(true)
      
      // Should highlight both 'Frontend' and 'React'
      const highlights = result.filter(part => 
    ...
    it('should escape special regex characters', () => {
      const result = highlightText('Price: $100.50 (discounted)', '$100.50') as React.ReactNode[]
    ...
    it('should handle parentheses and special characters', () => {
      ...
      testCases.forEach(({ text, query }) => {
        ...
        const highlightedPart = result.find(part => 
          React.isValidElement(part) && part.props.children === query
        ) as React.ReactElement
    ...
    it('should use custom styling', () => {
      ...
      const highlightedPart = result.find(part => 
        React.isValidElement(part) && part.props.children === 'World'
      ) as React.ReactElement
    ...
    it('should handle overlapping matches correctly', () => {
      const result = highlightText('AAA BBB AAA', 'AAA BBB') as React.ReactNode[]
    ...
    it('should handle very long text efficiently', () => {
      const longText = 'Lorem ipsum '.repeat(1000) + 'Developer' + ' Lorem ipsum '.repeat(1000)
    ...
    it('should handle search query with multiple spaces', () => {
      const result = highlightText('Frontend    Backend Developer', 'Frontend Backend') as React.ReactNode[]
